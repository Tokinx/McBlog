(function() {
	var WMD, Chunk, InputState, Command, Dialog, Overlay, Form, Field, LinkHelper, Translator, documentElement, eventCache = [],
	browser = {
	...
	WMD = function(input, toolbar, options) {
		...
		function createEvents() {
			var onSubmit;
			addEvent(input, browser.Opera ? "keypress": "keydown",
			function(event) {
	...
	Translator_EN = {
		...
		imageAlt: "请输入图片描述",
		listItem: "列表项目",
		strongTitle: "加粗 <strong> Ctl+B",
		emTitle: "斜体 <em> Ctl+I",
		aTitle: "插入链接 <a> Ctl+L",
		blockquoteTitle: "引用 <blockquote> Ctl+Q",
		codeTitle: "代码 <pre><code> Ctl+K",
		imgTitle: "插入图片 <img> Ctl+G",
		olTitle: "数字列表 <ol> Ctl+O",
		ulTitle: "普通列表 <ul> Ctl+U",
		hTitle: "标题 <h1>/<h2> Ctl+H",
		hrTitle: "分割线 <hr> Ctl+R",
		strongText: "Bold",
		emText: "Italic",
		aText: "Link",
		...
		insertImage: "插入图片",
		imageURL: "请在下方的输入框内输入要插入的远程图片地址",
		enterCodeHere: "请输入代码",
		blockquote: "引用文字",
		insertLink: "插入链接",
		linkURL: "请在下方的输入框内输入要插入的链接地址",
		submit: "确定",
		cancel: "取消"
	};
	Translator_CN = Translator_EN;
	Translator = Translator_CN;
		...
		return extend(obj, {
			...
			setTags: function(startExp, endExp) {
				...
				if (startExp) {
					...
					this.before = this.before.replace(tempExp,
					function(match) {
						that.startTag = that.startTag + match;
						return "";
					});
					tempExp = extendRegExp(startExp, "^", "");
					this.selection = this.selection.replace(tempExp,
					function(match) {
						that.startTag = that.startTag + match;
						return "";
					});
				...
				if (endExp) {
					...
					tempExp = extendRegExp(endExp, "^", "");
					this.after = this.after.replace(tempExp,
					function(match) {
						that.endTag = match + that.endTag;
						return "";
					});
	...
	extend(Command, {
		...
		autoIndent: function(wmd, chunk, callback, args) {
			...
			chunk.before = chunk.before.replace(/(\n|^)[ \t]+\n$/, "\n\n");
			if (/(\n|^)[ ]{0,3}([*+-])[ \t]+.*\n$/.test(chunk.before)) {
				Command.runners.ul(wmd, chunk, callback, extend(args, {
					preventDefaultText: false
				}));
			} else {
				if (/(\n|^)[ ]{0,3}(\d+[.])[ \t]+.*\n$/.test(chunk.before)) {
					Command.runners.ol(wmd, chunk, callback, extend(args, {
						preventDefaultText: false
					}));
				} else {
					if (/(\n|^)[ ]{0,3}>[ \t]+.*\n$/.test(chunk.before)) {
		...
		runList: function(wmd, chunk, callback, args) {
			...
			bullet = "-",
			num = 1,
			hasDigits, nLinesBefore, prefix, nLinesAfter, spaces;
		...
		runners: {
			...
			h: function(wmd, chunk, callback, args) {
				...
				callback = typeof callback === "function" ? callback: function() {};
				var headerLevel = 0,
				headerLevelToCreate, headerChar, len;
				chunk.selection = chunk.selection.replace(/\s+/g, " ");
				chunk.selection = chunk.selection.replace(/(^\s+|\s+$)/g, "");
				if (!chunk.selection) {
					chunk.startTag = "## ";
					chunk.selection = "标题文字";
					chunk.endTag = " ##";
				} else {
					chunk.setTags(/#+[ ]*/, /[ ]*#+/);
	...
	Dialog = function(options) {
		var obj, element, overlay, events = [],
		options = extend({
			zIndex: 10,
			css: "wmd-dialog",
			overlayColor: "#FFFFFF",
	...
	Overlay = function(options) {
		...
		function build() {
			if (!element) {
				...
				if (browser.IE) {
					element.style.filter = "progid:DXImageTransform.Microsoft.Alpha(opacity=" + (options.opacity * 50) + ")";
	...
	Field = function(label, type, options) {
		...
		options);
		var element, labelElement, inner, inputs, errorElement, events = [],
		setFor = false;
	...
	extend(Field, {
		TYPES: ["button", "checkbox", "empty", "file", "hidden", "image", "password", "radio", "reset", "submit", "text", "select", "textarea"],
		...
		createSelectList: function(parent, type, options) {
			var i, n, id = Field.getInputId(options),
			select,
			index;
	...
	LinkHelper = {
		...
		createDialog: function(formTitle, fieldLabel, callback) {
			...
			callback = typeof callback === "function" ? callback: function() {};
			form = Command.createSubmitCancelForm(formTitle,
			function() {
