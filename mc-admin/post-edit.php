...
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
  ...
  <div style="margin-bottom:20px;">
    ...
    <select name="year">
      <option value=""></option>
<?php $year = substr($post_date, 0, 4); for ($i = date("Y")-3; $i <= date("Y")+3; $i ++) { ?>
...
<?php if ($succeed) {
        if ($post_state == 'publish')
            echo '<div class="updated">文章已发布。 <a href="'.$mc_config['site_link'].'/?post/'.$post_id.'" target="_blank">查看文章</a></div>';
        else
